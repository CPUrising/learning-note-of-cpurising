...
// 坐标结构体（模拟校园/社区小范围坐标，单位：km）
struct Coordinate {
    double x;  // 横坐标（比如 116.404-116.406）
    double y;  // 纵坐标（比如 39.902-39.904）
...
Order generateRandomOrder(int orderIdx, time_t currentTime) {
    ...
    // 模拟校园坐标范围（116.404-116.406, 39.902-39.904）
    random_device rd;
    mt19937 gen(rd());
    uniform_real_distribution<> xDist(116.404, 116.406);
    uniform_real_distribution<> yDist(39.902, 39.904);
    uniform_real_distribution<> amountDist(15.0, 40.0);  // 订单金额15-40元
    uniform_int_distribution<> cookTimeDist(3, 9);       // 出餐时间3-9分钟
    uniform_int_distribution<> remainTimeDist(5, 15);    // 剩余送达时间5-15分钟

    order.merchantPos = Coordinate(xDist(gen), yDist(gen));  // 商家位置（食堂/商铺）
    order.userPos = Coordinate(xDist(gen), yDist(gen));      // 用户位置（宿舍/居民楼）
    order.orderAmount = round(amountDist(gen) * 10) / 10;    // 保留1位小数
    order.avgCookTime = cookTimeDist(gen);
    order.createTime = currentTime;
    order.requireArriveTime = currentTime + remainTimeDist(gen) * 60;  // 转换为秒
...
int main() {
    ...
    RiderStatus riderStatus(Coordinate(116.405, 39.903), 0, 100);  // 骑手初始在校园中心
    ...
    vector<Order> orders;
    for (int i = 0; i < 10; ++i) {
        orders.push_back(generateRandomOrder(i + 1, currentTime));
        orderHeap.pushOrder(&orders[i], riderStatus, currentTime);
        cout << "订单ID: " << orders[i].orderId
            << " | 金额: " << orders[i].orderAmount << "元"
            << " | 剩余时间: " << orders[i].getRemainingTime(currentTime) << "分钟"
            << " | 商家距离: " << orders[i].calcDistanceToMerchant(riderStatus.currentPos) << "km" << endl;
    }
